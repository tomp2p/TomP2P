...
/**
 * The message class manages a message that is being sent to a NAT-PMP gateway. The class respects the back-off time
 * defined in the NAT-PMP specification. Consumers of this class need to ensure that only one message is being sent at a
 * time to remain compliant with the NAT-PMP specification. This class is thread-safe.
 * 
...
public abstract class Message
{
     ...
     * 
     * @param type The {@link MessageType} of the message. This must not be null.
    ...
    /**
     * Gets a byte array initialized with the request packet payload. The version and opcode fields are overwritten when
     * the packet is sent, therefore it is not necessary to initialize them in this method.
     * 
    ...
    /**
     * Parses the response. This method is automatically called once after a message is received.
     * 
     * @throws Throwable Any exception may be thrown by the implementer.
     */
    abstract void parseResponse( byte[] response )
        throws Throwable;
    ...
    /**
     * Sets the exception associated with the response. This exception is generally provided by an external entity.
     * 
     * @param responseException The {@link NatPmpException} to associate with the response.
     */
    ...
    /**
     * Parses an unsigned integer (long) from a byte array in network byte order.
     * 
    ...
    {
        return (int) ( ( src[offset] & 0xFF ) << 24 ) + ( ( src[offset + 1] & 0xFF ) << 16 )
    ...
    /**
     * Sends the message. The sending process is synchronous. The NAT-PMP protocol specifies back-off times that are
     * respected by this implementation. Each message object may only be sent once.
     * 
     * @param destination The destination address.
     * @throws NatPmpException Thrown when this message is already being sent or if there is a problem setting up the
     *             {@link DatagramSocket}.
     */
    ...
    private void sendMessageInternal( InetAddress destination )
        throws NatPmpException
    {
