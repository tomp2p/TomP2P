...
public class ShutdownBuilder extends DHTBuilder<ShutdownBuilder> {
    private static final FutureShutdown FUTURE_SHUTDOWN = new FutureShutdown(null)
            .failed("shutdown builder - peer is shutting down");
     ...
     */
    public ShutdownBuilder(final PeerDHT peer) {
        super(peer, peer.peer().peerID());
        self(this);
    }
    ...
    public FutureShutdown start() {
        ...
        return peer.distributedHashTable().quit(this);
    }

    @Override
    public ShutdownBuilder domainKey(final Number160 domainKey) {
