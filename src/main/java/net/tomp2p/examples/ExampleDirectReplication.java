...
public class ExampleDirectReplication
{
    ...
    private static void exmpleDirectReplication( Peer[] peers )
        throws IOException
    {
        ...
        FutureDHT futureDHT =
            peers[1].put( Number160.ONE ).setData( new Data( "test" ) ).setFutureCreate( futureCreate1 ).setRefreshSeconds( 2 ).start();
        ...
        futureDHT =
            peers[1].remove( Number160.ONE ).setFutureCreate( futureCreate2 ).setRefreshSeconds( 2 ).setRepetitions( 2 ).start();
        Timings.sleepUninterruptibly( 9 * 1000 );
    }
