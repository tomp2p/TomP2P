...
/**
 * MessageQueue manages a queue of {@link Message}s and ensures that only one message at time is sent to the NAT-PMP
 * device. To use the MessageQueue, simply call {@link createMesageQueue()}. This will create and return a MessageQueue.
 * {@link shutdown()} must be called when the queue is no longer needed. This class is thread-safe.
 * 
 * @author flszen
 */
class MessageQueue
    implements Runnable
{
    ...
    /**
     * Creates and starts a {@link MessageQueue}. The created MesageQueue is returned. To use the queue, add
     * {@link Message}s through the {@link enqueueMessage(Message)} method.
     * 
    ...
    /**
     * Clears all messages in the queue. If a message is currently being processed, it is not interrupted.
     */
    void clearQueue()
    {
        synchronized ( queueLock )
    ...
    /**
     * Shuts down this messageQueue. This method is synchronous. The queue is cleared and it waits for the last item to
     * be processed.
     */
    void shutdown()
    {
