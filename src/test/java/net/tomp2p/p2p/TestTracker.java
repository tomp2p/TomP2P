...
public class TestTracker {
    ...
    public void testTracker() throws Exception {
        ...
        Peer master = null;
        try {
            master = new PeerMaker(new Number160(rnd)).setP2PId(1).setConfiguration(CONFIGURATION).setPorts(4001)
                    .makeAndListen();
            ...
            // null, rc, tc);
            FutureTracker ft = nodes[300].addTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
            ...
            ft = nodes[301].getTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
    ...
    public void testTracker3() throws Exception {
        ...
        try {
            ...
            ft.awaitUninterruptibly();
            ft = nodes[301].addTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc).start();
            System.err.println("add the peer to the tracker: " + nodes[301].getPeerAddress());
            ft.awaitUninterruptibly();
            ft = nodes[302].addTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc).start();
            ...
            ft = nodes[299].getTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc)
                    .setEvaluatingScheme(new VotingSchemeTracker()).start();
            ft.awaitUninterruptibly();
            Assert.assertEquals(true, ft.isSuccess());
    ...
    public void testTracker4() throws Exception {
        ...
        try {
            ...
            FutureTracker ft1 = nodes[299].getTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc)
                    .setEvaluatingScheme(new VotingSchemeTracker()).start();
            ...
            FutureTracker ft2 = nodes[299].getTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc)
                    .setEvaluatingScheme(new VotingSchemeTracker()).setKnownPeers(ft1.getKnownPeers()).start();
    ...
    public void testTracker5() throws Exception {
        ...
        try {
            ...
            FutureTracker ft = nodes[300].addTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc)
                    .setAttachement(new String(",.peoueuaoeue").getBytes()).start();
            ft.awaitUninterruptibly();
            ft = nodes[301].addTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc)
                    .setAttachement(new String(",.peoueuaoeue").getBytes()).start();
            ...
            ft = nodes[302].addTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    ...
                    .setAttachement(new String(",.peoueuaoeue").getBytes()).start();
            ft.awaitUninterruptibly();
            ft = nodes[303].addTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
                    .setRoutingConfiguration(rc).setTrackerConfiguration(tc)
                    .setAttachement(new String(",.peoueuaoeue").getBytes()).start();
            ft.awaitUninterruptibly();
            Assert.assertEquals(true, ft.isSuccess());
            Assert.assertEquals(2, ft.getDirectTrackers().size());
            tc = new TrackerConfiguration(1, 1, 0, 1);
            ft = nodes[199].getTracker(trackerID).setDomainKey(new ShortString("test").toNumber160())
    ...
    private Peer[] createNodes(Peer master, int nr, Random rnd) throws Exception {
        ...
        for (int i = 0; i < nr; i++) {
            nodes[i] = new PeerMaker(new Number160(rnd)).setP2PId(1).setConfiguration(CONFIGURATION)
                    .setMasterPeer(master).makeAndListen();
        }
        return nodes;
    }
}