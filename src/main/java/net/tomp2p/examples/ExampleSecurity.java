...
public class ExampleSecurity
{
	...
	public static void exampleAllMaster() throws NoSuchAlgorithmException, IOException, ClassNotFoundException
	{
		...
		Peer peer1 = new PeerMaker(pair1).setPorts(4001).buildAndListen();
		...
		FutureDHT futureDHT = peer1.put(Number160.ONE).setData(new Data("test")).setDomainKey(peer2Owner).setProtectDomain().build();
		...
		futureDHT = peer3.put(Number160.ONE).setData(new Data("hello")).setDomainKey(peer2Owner).setProtectDomain().build();
		futureDHT.awaitUninterruptibly();
		System.out.println("stored: "+futureDHT.isSuccess()+" -> becaues peer1 already claimed this domain");
		//peer 2 claims this domain
		futureDHT = peer2.put(Number160.ONE).setData(new Data("MINE!")).setDomainKey(peer2Owner).setProtectDomain().build();
		futureDHT.awaitUninterruptibly();
		System.out.println("stored: "+futureDHT.isSuccess()+" -> becaues peer2 is the owner");
		//get the data!
		futureDHT = peer1.get(Number160.ONE).setDomainKey(peer2Owner).build();
		futureDHT.awaitUninterruptibly();
	...
	public static void exampleNoneMaster() throws NoSuchAlgorithmException, IOException, ClassNotFoundException
	{
		...
		//peer 1 stores "test" in the domain key of owner peer 2 
		FutureDHT futureDHT = peer1.put(Number160.ONE).setData(new Data("test")).setProtectDomain().setDomainKey(peer2Owner).build();
		futureDHT.awaitUninterruptibly();
		//peer 2 did not claim this domain, so we stored it
		System.out.println("stored: "+futureDHT.isSuccess()+" -> because no one can claim domains except the owner, storage ok but no protection");
		//peer 3 want to store something
		futureDHT = peer3.put(Number160.ONE).setData(new Data("hello")).setProtectDomain().setDomainKey(peer2Owner).build();
		futureDHT.awaitUninterruptibly();
		System.out.println("stored: "+futureDHT.isSuccess()+" -> because no one can claim domains except the owner, storage ok but no protection");
		//peer 2 claims this domain
		futureDHT = peer2.put(Number160.ONE).setData(new Data("MINE!")).setProtectDomain().setDomainKey(peer2Owner).build();
		futureDHT.awaitUninterruptibly();
		System.out.println("stored: "+futureDHT.isSuccess()+" -> becaues peer2 is the owner");
		//get the data!
		futureDHT = peer1.get(Number160.ONE).setDomainKey(peer2Owner).build();
		futureDHT.awaitUninterruptibly();
		System.out.println("we got "+futureDHT.getData().getObject());
		futureDHT = peer3.put(Number160.ONE).setDomainKey(peer2Owner).setData(new Data("hello")).build();
		futureDHT.awaitUninterruptibly();
	...
	private static void setProtection(Peer[] peers, ProtectionEnable protectionEnable, ProtectionMode protectionMode)
	{
		for(Peer peer:peers)
		{
			peer.getPeerBean().getStorage()
				.setProtection(protectionEnable, protectionMode,
						protectionEnable, protectionMode);
		}
	}
}
