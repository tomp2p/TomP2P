...
/**
 * Testing with -Djava.util.logging.config.file=src/main/config/tomp2plog.properties -server -Xms2048m -Xmx2048m -XX:+UseParallelGC -XX:+AggressiveOpts -XX:+UseFastAccessorMethods
 * @author draft
 *
 */
public class TestPerformance
{
	...
	private static void test1() throws Exception
	{
		...
		try
		{
			...
			for(int i=0;i<50;i++)
			{
				...
				PeerConnection peerConnection = peers[indexCur].createPeerConnection(peers[indexNext].getPeerAddress(), 1000);
			...
			{
				int indexCur = i%50;
				FutureResponse response = peers[indexCur].sendDirect().setConnection(peerConnections[indexCur]).setObject(buffer).build();
				result.add(response);
			}
	...
	private static void test2() throws Exception
	{
		...
		try
		{
			...
			for(int i=0;i<50;i++)
			{
				FutureChannelCreator futureChannelCreator = peers[i].getConnectionBean().getConnectionReservation().reserve(10);
			...
			{
				int index = i%50;
				FutureResponse response = peers[index].getHandshakeRPC().pingTCP(peers[(i+1)%50].getPeerAddress(), channelCreators[index]);
				result.add(response);
			}
			for(FutureResponse futureResponse:result)
			{
	...
	private static void test3() throws Exception
	{
		...
		try
		{
			...
			for(int i=0;i<50;i++)
			{
				FutureChannelCreator futureChannelCreator = peers[i].getConnectionBean().getConnectionReservation().reserve(1);
				futureChannelCreator.awaitUninterruptibly();
				channelCreators[i]=futureChannelCreator.getChannelCreator();
			}
			long start = System.currentTimeMillis();
			...
			{
				int index = i%50;
				FutureResponse response = peers[index].getHandshakeRPC().pingUDP(peers[(i+1)%50].getPeerAddress(), channelCreators[index]);
				result.add(response);
			}
			for(FutureResponse futureResponse:result)
			{
	...
	private static void test4() throws Exception
	{
		...
		try
		{
			...
			{
				FutureChannelCreator futureChannelCreator = peers[i].getConnectionBean().getConnectionReservation().reserve(50);
				futureChannelCreator.awaitUninterruptibly();
				channelCreators[i]=futureChannelCreator.getChannelCreator();
			}
			long start = System.currentTimeMillis();
			for(int i=0;i<nrOfRequests;i++)
			{
				FutureResponse response = peers[0].getHandshakeRPC().pingTCP(peers[1].getPeerAddress(), channelCreators[0]);
				result.add(response);
			}
			for(FutureResponse futureResponse:result)
			{
