...
/**
 * Registration Service for {@link RegistrationBitcoin} class.
 *
 * @author Alexander Mülli
 *
 */
public class RegistrationBitcoinService implements RegistrationService<RegistrationBitcoin> {

    private final static Logger LOG = LoggerFactory.getLogger(RegistrationBitcoinService.class);
     ...
     */
    public RegistrationBitcoinService(RegistrationStorage storage, NetworkParameters params, File dir, String filename) {
        this.storage = storage;
        this.kit = new WalletAppKit(params, dir, filename);
    }

    @Override
    public RegistrationBitcoinService start() {
        storage.start();
        kit.startAsync();
...
//        tx.addOutput(Transaction.MIN_NONDUST_OUTPUT, script);
            ...
            public void onConfidenceChanged(Transaction tx, ChangeReason reason) {
                ...
                if (reason.equals(ChangeReason.TYPE) && confidence.getConfidenceType().equals(TransactionConfidence.ConfidenceType.BUILDING)) {
                    ...
                    try {
                        ...
                        LOG.debug("registration of peer {} complete", registration.getPeerId());
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
    ...
    public boolean verify(RegistrationBitcoin registration) {
        ...
        LOG.debug("registration of peer {} unknown or has changed", peerId);
        ...
        Block b = null;
        // Use filteredBlock instead and only download relevant transaction
        try {
