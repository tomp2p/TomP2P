...
/**
 * Gathers information about interface bindings. Here a user can set the preferences to which addresses to bind the
 * socket. This class contains two types of information: 1.) the interface/address to listen for incoming connections
 * and 2.) how other peers see us. The default is to listen to all interfaces and our outside address is set to the
 * first interface it finds. If more than one search hint is used, then the combination operation will be "and"
 * 
 * @author Thomas Bocek
 */
...
{
    /**
     * Types of protocols. This was a boolean, but there needs to be the type "Any".
     */
    public enum Protocol
    {
        ...
        IPv6, 
        /**
         * This type indicates that the address to bind to can be either IPv4 or IPv6
         */
        Any
    };
    ...
    // This can be set by the user. The discover process will not use this field to store anything
     ...
     * 
     * @param protocol The protocol to bind to
     * @param externalAddress The external address, how other peers will see us. Use null if you don't want to use
     *            external address
     * @param externalTCPPort The external port, how other peers will see us, if 0 is provided, a random port will be
     *            used
     * @param externalUDPPort The external port, how other peers will see us, if 0 is provided, a random port will be
     *            used
     */
    ...
    /**
     * Returns a list of InetAddresses to listen to. First Inet4Addresses, then Inet6Addresses are present in the list.
     * This returns the matching addresses from DiscoverNetworks.
     * 
    ...
    /**
     * Adds the results from an other binding. This is useful because you can add within one Bindings hints only with
     * "and", with add() you have the option "or" as well. E.g., Bindings b1 = new Bindings(IPv4, eth0); Bindings b2 =
     * new Bindings(IPv6, eth1); b2.add(b1) -> this will bind to all IPv4 addresses on eth0 and all IPv6 addresses on
     * eth1
     * 
    ...
    /**
     * @return True if the user specified both ports in advance. This tells us that the user knows about the ports and
     *         did a manual port-forwarding.
     */
