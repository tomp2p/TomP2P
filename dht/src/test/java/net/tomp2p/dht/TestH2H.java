...
public class TestH2H {
	...
	public void testVersionFork() throws Exception {
		KeyPairGenerator gen = KeyPairGenerator.getInstance("DSA");
		KeyPair keyPair1 = gen.generateKeyPair();
		PeerDHT p1 = new PeerBuilderDHT(
				new PeerBuilder(Number160.createHash(1)).ports(4838).start())
				.start();
		PeerDHT p2 = new PeerBuilderDHT(
		...
		Data versionA = new Data("versionA").addBasedOn(new Number160(0, Number160.ZERO))
				.protectEntry(keyPair1);
		Data versionB = new Data("versionB").addBasedOn(new Number160(0, Number160.ONE))
				.protectEntry(keyPair1);
		FuturePut putA = p1.put(locationKey)
				.data(contentKey, versionA, Number160.ONE).keyPair(keyPair1)
		...
		FuturePut putB = p1.put(locationKey)
				.data(contentKey, versionB, Number160.ONE).keyPair(keyPair1)
				.start().awaitUninterruptibly();
