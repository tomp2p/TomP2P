...
public class ChannelCreator
{
    ...
    ChannelCreator( int permits, Statistics statistics, MessageLogger messageLoggerFilter,
                    ChannelFactory tcpClientChannelFactory, ChannelFactory udpClientChannelFactory,
                    boolean keepAliveAndReuse, String name, long creatorThread, Scheduler scheduler )
    {
    ...
    private void connectionNotReadyYetTCP( final FutureChannel futureChannelCreation,
                                           ...
                                           final RequestHandlerTCP<? extends BaseFuture> requestHandler,
                                           final int connectTimeoutMillis, final InetSocketAddress recipient,
                                           final Semaphore connectionSemaphore2 )
    {
        scheduler.addConnectionQueue( futureChannelCreation, connectionSemaphore2, new Runnable()
        {
            ...
            public void run()
            {
                createTCPChannel( futureChannelCreation, timeoutHandler, requestHandler, connectTimeoutMillis,
                                  recipient );
            }
        } );
    }
    ...
    private void connectionNotReadyYetUDP( final FutureChannel futureChannelCreation,
                                           final ReplyTimeoutHandler timeoutHandler,
                                           final RequestHandlerUDP<? extends BaseFuture> requestHandler,
    ...
    /**
     * Creates a channel the Netty way. We set soLinger to 0 since we may end up with too many connections in a WAIT
     * state. Setting soLinger to 0 sends back an RST in case of a close, which may get an exception
     ...
     * @param connectionTimoutMillis The timeout after which a connection attempt is considered a failure
     * @return The channel future
     */
    ...
    /**
     * Creates a channel the Netty way. We need to set the receive buftfer, since we need to reserve enough space and
     * the default 786 bytes is not enough.
     * 
     * @param timeoutHandler The handler that deals with timeouts
     * @param requestHandler The handler that deals with incoming replies
    ...
    /**
     * Fill the TCP pipeline with handlers. This pipeline contains the streamer, which the UDP version doesnt.
     * 
     ...
     * 
     * @param destination The address of the destination peer of the permanent connection.
     * @return The ChannelFuture of the close operation or null if the connection was not in the cached map.
     */
    ...
    /**
     * @return The number of permits, which is the max. number of allowed parallel connections
     */
