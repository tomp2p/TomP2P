...
/**
 * The decoder first decodes the header. If this has been successful, then if there is payload, then the payload is
 * decoded. The TomP2P decoder can be used to decode several messages in the same session as long as they are
    ...
    extends FrameDecoder
{
    ...
    /**
     * {@inheritDoc} decodeLast is called from cleanup within FrameDecoder, which is triggered by a close. This can be
     * called by a timer if and IdleHandler has been defined.
     */
    /*
     * @Override protected Object decodeLast(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws
     * Exception { return null; }
     */

    /**
     * After successfully reception of the message, we need to set message to null in order to reuse the connection. If
     * connection is closed, then this cleanup would not be necessary as the annotation ChannelPipelineCoverage("one")
     * creates a new TomP2PDecoder. Thus, we do not need to cleanup in the close() or disconnect() method.
     * 
