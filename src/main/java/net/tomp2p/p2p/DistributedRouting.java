...
public class DistributedRouting
{
     ...
     * 
     * @param peerAddresses the node to which bootstrap should be performed to
     * @param maxNoNewInfo number of nodes asked without new information to stop at
    ...
    public FutureWrapper<FutureRouting> bootstrap( final Collection<PeerAddress> peerAddresses, final int maxNoNewInfo,
                                                   final int maxFailures, final int maxSuccess, final int parallel,
                                                   final boolean forceTCP, final boolean isForceRoutingOnlyToSelf,
                                                   final ChannelCreator cc )
    {
        ...
        FutureRouting futureRouting =
            routing( peerAddresses, peerBean.getServerPeerAddress().getID(), null, null, 0, maxNoNewInfo, maxFailures,
        ...
        futureRouting.addListener( new BaseFutureAdapter<FutureRouting>()
        {
                ...
                throws Exception
            {
                ...
                FutureRouting futureRouting =
                    routing( peerAddresses, null, null, null, 0, maxNoNewInfo, maxFailures, maxSuccess, parallel,
                             Type.REQUEST_1, forceTCP, cc, true, isForceRoutingOnlyToSelf );
     ...
     * @param domainKey the domain of the network the current node and locationKey is in
     * @param contentKeys keys of the content to search for. Only used if you perform a get
     * @param maxDirectHits number of direct hits to stop at
     * @param maxNoNewInfo number of nodes asked without new information to stop at
     * @param maxFailures number of failures to stop at
     * @param parallel number of routing requests performed concurrently
     * @param isDigest Set to true to return a digest of the remote content for close neighbors
     * @param forceTCP Set to true if routing should use TCP connections
     * @param cc the channel creator
     * @return a FutureRouting object, is set to complete if the route has been found
     */
    public FutureRouting route( final Number160 locationKey, final Number160 domainKey,
                                final Collection<Number160> contentKeys, Type type, int maxDirectHits,
                                int maxNoNewInfo, int maxFailures, int maxSuccess, int parallel, boolean forceTCP,
                                final ChannelCreator cc )
    {
    ...
    private FutureRouting routing( Collection<PeerAddress> peerAddresses, Number160 locationKey,
                                   ...
                                   int maxDirectHits, int maxNoNewInfo, int maxFailures, int maxSuccess,
                                   final int parallel, Type type, boolean forceTCP, final ChannelCreator cc,
                                   final boolean isBootstrap, final boolean isForceRoutingOnlyToSelf )
    {
        ...
        // this balances load.
        final SortedMap<PeerAddress, DigestInfo> directHits =
            new TreeMap<PeerAddress, DigestInfo>( peerBean.getPeerMap().createPeerComparator() );
        ...
        else
        {
            ...
            boolean isRoutingOnlyToSelf =
                isForceRoutingOnlyToSelf
                    || ( peerAddresses.size() == 1 && peerAddresses.iterator().next().equals( peerBean.getServerPeerAddress() ) );
            routingRec( futureResponses, futureRouting, queueToAsk, alreadyAsked, directHits, potentialHits,
                        new AtomicInteger( 0 ), new AtomicInteger( 0 ), new AtomicInteger( 0 ), maxDirectHits,
                        maxNoNewInfo, maxFailures, maxSuccess, parallel, locationKey, domainKey, contentKeys, true,
                        type, forceTCP, false, cc, isBootstrap, !isRoutingOnlyToSelf );
        }
    ...
    private void routingRec( final FutureResponse[] futureResponses, final FutureRouting futureRouting,
                             ...
                             final Number160 locationKey, final Number160 domainKey,
                             ...
                             final ChannelCreator channelCreator, final boolean isBootstrap,
        ...
        for ( int i = 0; i < parallel; i++ )
        {
            if ( futureResponses[i] == null && !stopCreatingNewFutures )
            {
                ...
                if ( next != null )
                {
                    ...
                    futureResponses[i] =
                        neighbors.closeNeighbors( next, locationKey2, domainKey, contentKeys, type, channelCreator,
                                                  forceTCP );
                    if ( logger.isDebugEnabled() )
                    {
        ...
        fp.addListener( new BaseFutureAdapter<FutureForkJoin<FutureResponse>>()
        {
                ...
                throws Exception
            {
                ...
                if ( future.isSuccess() )
                {
                    ...
                    if ( logger.isDebugEnabled() )
                    {
                        logger.debug( "Peer (" + ( resultSize > 0 ? "direct" : "none" ) + ") " + remotePeer
                            + " reported " + newNeighbors );
                    }
                ...
                {
                    routingRec( futureResponses, futureRouting, queueToAsk, alreadyAsked, directHits, potentialHits,
                                nrNoNewInfo, nrFailures, nrSuccess, maxDirectHits, maxNoNewInfo, maxFailures,
                                maxSucess, parallel, locationKey, domainKey, contentKeys, cancelOnFinish, type,
                                forceTCP, stopCreatingNewFutures, channelCreator, isBootstrap, isRoutingToOthers );
                }
            }
        } );
    }
    ...
    static boolean evaluateDirectHits( Map<Number160, Number160> keyMap, PeerAddress remotePeer,
                                       final Map<PeerAddress, DigestInfo> directHits, DigestInfo digestBean,
                                       int maxDirectHits )
    {
        if ( digestBean.getSize() > 0 )
        {
    ...
    static boolean evaluateInformation( Collection<PeerAddress> newNeighbors, final SortedSet<PeerAddress> queueToAsk,
                                        final Set<PeerAddress> alreadyAsked, final AtomicInteger noNewInfo,
                                        int maxNoNewInfo )
    {
     ...
     * @param newPeers The new peers reported from remote peers. Since the remote peers do not know what we know, we
     ...
     * @param first The element to check if it will be the highest in the sorted set
