...
public class TestTracker
{
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            FutureResponse fr =
                sender.getTrackerRPC().addToTracker( recv1.getPeerAddress(), loc, dom, null, false, false, bloomFilter,
                                                     cc, false, false );
            fr.awaitUninterruptibly();
            Assert.assertEquals( true, fr.isSuccess() );
            bloomFilter = new SimpleBloomFilter<Number160>( 4096, 1000 );
            fr =
                sender.getTrackerRPC().getFromTracker( recv1.getPeerAddress(), loc, dom, false, false, bloomFilter, cc );
    ...
    @Test
    public void testTrackerPutNoBloomFilter()
        throws Exception
    {
        Peer sender = null;
        Peer recv1 = null;
        try
        {
            ...
            ChannelCreator cc = fcc.getChannelCreator();
            FutureResponse fr =
                sender.getTrackerRPC().addToTracker( recv1.getPeerAddress(), loc, dom, null, false, false, null, cc,
                                                     false, false );
            fr.awaitUninterruptibly();
            Assert.assertEquals( true, fr.isSuccess() );
            fr = sender.getTrackerRPC().getFromTracker( recv1.getPeerAddress(), loc, dom, false, false, null, cc );
            fr.awaitUninterruptibly();
            System.err.println( fr.getFailedReason() );
            Assert.assertEquals( true, fr.isSuccess() );
            PeerAddress peerAddress = fr.getResponse().getTrackerData().iterator().next().getPeerAddress();
    ...
    @Test
    public void testTrackerPutAttachment()
        throws Exception
    {
        Peer sender = null;
        Peer recv1 = null;
        try
        {
            ...
            ChannelCreator cc = fcc.getChannelCreator();
            FutureResponse fr =
                sender.getTrackerRPC().addToTracker( recv1.getPeerAddress(), loc, dom, new String( "data" ).getBytes(),
                                                     false, false, null, cc, false, false );
            fr.awaitUninterruptibly();
            Assert.assertEquals( true, fr.isSuccess() );
            fr = sender.getTrackerRPC().getFromTracker( recv1.getPeerAddress(), loc, dom, false, false, null, cc );
            fr.awaitUninterruptibly();
    ...
    @Test
    public void testTrackerBloomFilter()
        throws Exception
    {
        Peer sender = null;
        Peer recv1 = null;
        try
        {
            ...
            FutureResponse fr =
                sender.getTrackerRPC().addToTracker( recv1.getPeerAddress(), loc, dom, new String( "data" ).getBytes(),
                                                     false, false, bloomFilter, cc, false, false );
            fr.awaitUninterruptibly();
            Assert.assertEquals( true, fr.isSuccess() );
