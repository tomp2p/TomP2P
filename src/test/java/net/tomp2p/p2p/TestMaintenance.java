...
public class TestMaintenance {
    ...
    public void testMaintenance2() throws Exception {
        ...
        try {
            ...
            Collection<PeerAddress> pas = master.getPeerBean().getPeerMap().peersForMaintenance();
            // this needs to be empty because its within the 4 seconds, where we
            // do our second maintenance loop.
            Assert.assertEquals(0, pas.size());
            Timings.sleep(3000);
            pas = master.getPeerBean().getPeerMap().peersForMaintenance();
            // after 4 seconds we get all the peers back. 160 * bagsize is the
            // maximum capacity
    ...
    private Peer[] createNodes(Peer master, int nr, Random rnd) throws Exception {
        Peer[] peers = new Peer[nr];
        for (int i = 0; i < nr; i++) {
            peers[i] = new PeerMaker(new Number160(rnd)).setEnableMaintenance(false).setMasterPeer(master)
                    .makeAndListen();
        }
        return peers;
    }
