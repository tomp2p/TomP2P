...
public class NeighborRPC extends DispatchHandler {
	...
	private FutureResponse send(final Message message, final ConnectionConfiguration configuration,
						...
						if (ns != null) {
							for (PeerAddress neighbor : ns.neighbors()) {
								synchronized (peerBean().peerStatusListeners()) {
									for (PeerStatusListener peerStatusListener : peerBean()
											.peerStatusListeners()) {
										peerStatusListener.peerFound(neighbor, response.sender(), null);
									}
								}
							}
						}
	...
	public void handleResponse(final Message message, PeerConnection peerConnection, final boolean sign,
		...
		if (isDigest) {
			if (message.type() == Type.REQUEST_2) {
				...
				if (peerBean().digestStorage() == null) {
				...
				} else if ((keyBloomFilter != null || contentBloomFilter != null) && locationKey != null
						&& domainKey != null) {
					Number320 locationAndDomainKey = new Number320(locationKey, domainKey);
					...
					Number640 to = new Number640(locationAndDomainKey, Number160.MAX_VALUE,
							Number160.MAX_VALUE);
					digestInfo = peerBean().digestStorage().digest(from, to, -1, true);
				} else {
				...
				synchronized (peerBean().peerStatusListeners()) {
					for (PeerStatusListener peerStatusListener : peerBean().peerStatusListeners()) {
						peerStatusListener.peerFailed(message.sender(), new PeerException(
								AbortCause.SHUTDOWN, "shutdown"));
					}
				}
			}
		}
