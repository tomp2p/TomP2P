...
public class ExampleBloomFilter
{
    ...
    private static void exampleBloomFilter( Peer[] peers )
        throws IOException
    {
        ...
        FutureDHT futureDHT =
            peers[30].put( nr1 ).setDataMap( contentMap ).setDomainKey( Number160.createHash( "my_domain" ) ).start();
        ...
        futureDHT =
            peers[60].put( nr2 ).setDataMap( contentMap ).setDomainKey( Number160.createHash( "my_domain" ) ).start();
        futureDHT.awaitUninterruptibly();
        // digest the first entry
        futureDHT =
            peers[20].get( nr1 ).setDigest().setAll().setReturnBloomFilter().setDomainKey( Number160.createHash( "my_domain" ) ).start();
        futureDHT.awaitUninterruptibly();
        ...
        futureDHT =
            peers[10].get( nr2 ).setAll().setKeyBloomFilter( keyBF ).setDomainKey( Number160.createHash( "my_domain" ) ).start();
        futureDHT.awaitUninterruptibly();
