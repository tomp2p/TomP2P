...
public class ExampleSecurity
{
	...
	public static void exampleAllMaster() throws NoSuchAlgorithmException, IOException, ClassNotFoundException
	{
		...
		FutureDHT futureDHT = peer1.put(Number160.ONE).setData(new Data("test")).setDomainKey(peer2Owner).setProtectDomain().start();
		...
		futureDHT = peer3.put(Number160.ONE).setData(new Data("hello")).setDomainKey(peer2Owner).setProtectDomain().start();
		futureDHT.awaitUninterruptibly();
		System.out.println("stored: "+futureDHT.isSuccess()+" -> becaues peer1 already claimed this domain");
		//peer 2 claims this domain
		futureDHT = peer2.put(Number160.ONE).setData(new Data("MINE!")).setDomainKey(peer2Owner).setProtectDomain().start();
		futureDHT.awaitUninterruptibly();
	...
	public static void exampleNoneMaster() throws NoSuchAlgorithmException, IOException, ClassNotFoundException
	{
		...
		//peer 1 stores "test" in the domain key of owner peer 2 
		FutureDHT futureDHT = peer1.put(Number160.ONE).setData(new Data("test")).setProtectDomain().setDomainKey(peer2Owner).start();
		futureDHT.awaitUninterruptibly();
		//peer 2 did not claim this domain, so we stored it
		...
		//peer 3 want to store something
		futureDHT = peer3.put(Number160.ONE).setData(new Data("hello")).setProtectDomain().setDomainKey(peer2Owner).start();
		futureDHT.awaitUninterruptibly();
		System.out.println("stored: "+futureDHT.isSuccess()+" -> because no one can claim domains except the owner, storage ok but no protection");
		//peer 2 claims this domain
		futureDHT = peer2.put(Number160.ONE).setData(new Data("MINE!")).setProtectDomain().setDomainKey(peer2Owner).start();
		futureDHT.awaitUninterruptibly();
		System.out.println("stored: "+futureDHT.isSuccess()+" -> becaues peer2 is the owner");
		//get the data!
		futureDHT = peer1.get(Number160.ONE).setDomainKey(peer2Owner).start();
