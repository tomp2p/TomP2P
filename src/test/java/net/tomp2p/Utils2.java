...
public class Utils2 {
    /**
     * Used to make the testcases predictable. Used as an input for
     * {@link Random}.
     */
    public static final long THE_ANSWER = 42L;

    /**
     * Having two peers in a network, the seed needs to be different, otherwise
     * we create a peer with the same id twice.
     */
    ...
    public static Message createDummyMessage(boolean firewallUDP, boolean firewallTCP) throws UnknownHostException {
    ...
    public static Message createDummyMessage(Number160 idSender, String inetSender, int tcpPortSendor,
            int udpPortSender, Number160 idRecipien, String inetRecipient, int tcpPortRecipient, int udpPortRecipient,
            Command command, Type type, boolean firewallUDP, boolean firewallTCP) throws UnknownHostException {
        ...
        //
        PeerAddress n2 = createAddress(idRecipien, inetRecipient, tcpPortRecipient, udpPortRecipient, firewallUDP,
                firewallTCP);
     ...
     */
    public static Peer[] createNodes(int nrOfPeers, Random rnd, int port, int refresh, boolean enableIndirectReplication)
            throws Exception {
        if (nrOfPeers < 1) {
        ...
        if (refresh > 0) {
            if (enableIndirectReplication) {
                peers[0] = new PeerMaker(new Number160(rnd)).setRandom(rnd)
                        .setEnableIndirectReplication(enableIndirectReplication).setTcpPort(port).setUdpPort(port)
                        .setReplicationRefreshMillis(refresh).makeAndListen();
            } else {
                peers[0] = new PeerMaker(new Number160(rnd)).setRandom(rnd).setTcpPort(port).setUdpPort(port)
                        .setReplicationRefreshMillis(refresh).makeAndListen();
            }
        } else {
            if (enableIndirectReplication) {
        ...
        for (int i = 1; i < nrOfPeers; i++) {
            if (enableIndirectReplication) {
                peers[i] = new PeerMaker(new Number160(rnd)).setEnableIndirectReplication(enableIndirectReplication)
    ...
    public static Peer[] createRealNodes(int nrOfPeers, Random rnd, int startPort) throws Exception {
        ...
        for (int i = 0; i < nrOfPeers; i++) {
            peers[i] = new PeerMaker(new Number160(rnd)).setEnableMaintenance(false).setPorts(startPort + i)
                    .makeAndListen();
        }
    ...
    public static Peer[] createNonMaintenanceNodes(int nrOfPeers, Random rnd, int port) throws IOException {
        ...
        for (int i = 1; i < nrOfPeers; i++) {
            peers[i] = new PeerMaker(new Number160(rnd)).setEnableMaintenance(false).setMasterPeer(peers[0])
                    .makeAndListen();
        }
