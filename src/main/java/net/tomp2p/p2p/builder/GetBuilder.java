    ...
    extends DHTBuilder<GetBuilder>
{
    ...
    private final static FutureDHT FUTURE_DHT_MESSAGE_TOO_LARGE =
        new FutureDHT().setFailed( "Message size exceeds UDP transfer size, please consider using TCP" );
    ...
    public FutureDHT start()
    {
        ...
        return peer.getDistributedHashMap().get( locationKey, domainKey, contentKeys, keyBloomFilter, valueBloomFilter,
                                                 routingConfiguration, requestP2PConfiguration, evaluationScheme,
                                                 signMessage, digest, returnBloomFilter, range, manualCleanup,
     ...
     * In this case, the user has to use this method to check if {@link RoutingConfiguration} needs to have the forceTCP
     * flag.
     * 
