...
public class SimulateTracker
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		simulateTracker.simulationParametersPull(15,10,3);
		simulateTracker.simulationParametersPrimary(15,10,3);
		simulateTracker.simulationParametersPEX(15, 10, 3);
		simulateTracker.simulationParametersPullPex(15, 10, 3);
	}

	// measurements
	public void simulationParametersPull(int maxAskTrackers, int maxUploadTo, int maxRepeat) throws Exception
	{
		StringBuilder sb = new StringBuilder();
		int counter = 0;
		for (int askTrackers = 1; askTrackers <= maxAskTrackers; askTrackers++)
		{
			for (int uploadTo = 1; uploadTo <= maxUploadTo; uploadTo++)
			{
				...
				try
				{
					...
					for (int repetition = 1; repetition <= maxRepeat; repetition++)
					{
						counter++;
						FutureTracker ft;
						if (knownPeers != null)
						...
						System.err.println(">>>>" + counter + ">>>>" + askTrackers + ";" + uploadTo + ";" + repetition
								+ ";" + tmp.size() + ";");
						master.getConnectionHandler().customLoggerMessage(">>>>" + counter + ">>>>" + askTrackers + ";" + uploadTo + ";" + repetition + ";"
								+ tmp.size() + ";");
					}
				}
				finally
	...
	// measurements
	public void simulationParametersPrimary(int maxAskTrackers, int maxUploadTo, int maxRepeat) throws Exception
	{
		StringBuilder sb = new StringBuilder();
		int counter = 0;
		for (int askTrackers = 1; askTrackers <= maxAskTrackers; askTrackers++)
	...
	// measurements
	public void simulationParametersPEX(int maxAskTrackers, int maxUploadTo, int maxRepeat) throws Exception
	{
		StringBuilder sb = new StringBuilder();
		int counter = 0;
			...
			for (int askTrackers = 1; askTrackers <= maxAskTrackers; askTrackers++)
			{
				for (int uploadTo = 1; uploadTo <= maxUploadTo; uploadTo++)
				{
					...
					try
					{
						...
						tc = new TrackerConfiguration(1, 1, askTrackers, 301, 5, 30);
						ConfigurationTrackerGet ctg = Configurations.defaultTrackerGetConfiguration();
						ctg.setDomain(new ShortString("test").toNumber160());
						ctg.setRoutingConfiguration(rc);
						ctg.setTrackerConfiguration(tc);
						...
						for (int repetition = 1; repetition <= maxRepeat; repetition++)
						{
							...
							if (repetition == 1)
							{
								for (int i = 0; i <= 300; i++)
								{
									nodes[300+i].getFromTracker(trackerID, ctg).awaitUninterruptibly();
								}
								ft = nodes[299].getFromTracker(trackerID, ctg);
								ft.awaitUninterruptibly();
								// knownPeers = ft.getKnownPeers();
							...
							for (Number480 n480 : nodes[299].getPeerBean().getTrackerStorage()
									.getKeys(new Number320(trackerID, domain.xor(Number160.MAX_VALUE))))
							{
								tmp.remove(n480.getContentKey());
							}
							for (int i = 0; i <= 300; i++)
	...
	// measurements
	public void simulationParametersPullPex(int maxAskTrackers, int maxUploadTo, int maxRepeat) throws Exception
	{
		StringBuilder sb = new StringBuilder();
		int counter = 0;
				...
				for (int uploadTo = 1; uploadTo <= maxUploadTo; uploadTo++)
				{
					...
					try
					{
						...
						for (int repetition = 1; repetition <= maxRepeat; repetition++)
						{
							...
							for (int i = 0; i <= 300; i++)
							{
								nodes[600 - i].getTracker().startExchange(trackerID, ctg.getDomain());
							}
							Utils.sleep(500);
							for (Number480 n480 : nodes[299].getPeerBean().getTrackerStorage()
									.getKeys(new Number320(trackerID, ctg.getDomain())))
