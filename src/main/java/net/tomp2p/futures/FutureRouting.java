...
/**
 * The routing future keeps track of the routing process. This means that the routing future is returned immediately and
 * the routing process starts in the background. There are two ways to wait for the routing process: (1) to use await*,
 * which blocks the current thread and waits for the routing process to complete, or (2) to use addListener*, which will
 * be called when the routing process completes. The listener may or may not run in the same thread. The routing will
 * always succeed if we do DHT operations or bootstrap to ourself. It will fail if we bootstrap to another peer, but
 * could not contact any peer than ourself.
 * 
    ...
    extends BaseFutureImpl<FutureRouting>
{
    ...
    /**
     * Sets the result of the routing process and finishes the future. This will notify all listeners. The future will
     * always succeed if we do DHT operations or bootstrap to ourself. It will fail if we bootstrap to another peer, but
     * could not contact any peer than ourself.
     * 
     * @see #getDirectHits()
     ...
     * @see #getRoutingPath()
     * @param directHits The direct hits, the peers in the direct set that reports to have the key (Number160) we were
     *            looking for.
     * @param potentialHits The potential hits, the peers in the direct set and those peers that reports to *not* have
                              ...
                              boolean isBootstrap, boolean isRoutingToOther )
    {
        synchronized ( lock )
        {
            ...
            {
                // we need to fail if we only find ourself. This means that we did not connect to any peer and we did
                // not
                // wanted connect to ourself.
                this.type =
                    ( ( potentialHits.size() <= 1 ) && ( directHits.size() == 0 ) ) ? BaseFuture.FutureType.FAILED
    ...
    /**
     * The potential hits set contains those peers that are in the direct hit and that did report to *not* have the key
     * (Number160) we were looking for. We already check for the content during routing, since we send the information
     * what we are looking for anyway, so a reply if the content exists or not is not very expensive. However, a peer
     * may lie about this.
     * 
     * @see #getDirectHits()
     * @see #getDirectHitsDigest()
     * @return The potential hits, the peers in the direct set and those peers that reports to *not* have the key
    ...
    /**
     * The direct hits set contains those peers that reported to have the key (Number160) we were looking for. We
     * already check for the content during routing, since we send the information what we are looking for anyway, so a
     * reply if the content exists or not is not very expensive. However, a peer may lie about this.
     * 
     * @see #getPotentialHits()
     * @see #getDirectHitsDigest()
     * @return The direct hits, the peers in the direct set that reports to have the key (Number160) we were looking
     *         for.
     */
     ...
     * @see #getPotentialHits()
     * @see #getDirectHits()
     * @return The direct hits including its digest (size of the result set and its xored hashes), when a peer reports
     *         to have the key (Number160) we were looking for.
     */
    ...
    /**
     * Returns the peers that have been asked to provide neighbor information. The order is sorted by peers that were
     * close to the target.
     * 
