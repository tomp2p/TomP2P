    ...
    extends ReplyHandler
{
     ...
     * @param contentKeys For get() and remove() one can provide the content keys and the remote peer indicates if those
     *            keys are on that peer.
                                          ...
                                          boolean forceTCP )
    {
        ...
        if ( !forceTCP )
        {
            ...
            NeighborsRequestUDP<FutureResponse> request =
                new NeighborsRequestUDP<FutureResponse>( futureResponse, getPeerBean(), getConnectionBean(), message );
            return request.sendUDP( channelCreator );
        }
        else
        {
            FutureResponse futureResponse = new FutureResponse( message );
            NeighborsRequestTCP<FutureResponse> request =
                new NeighborsRequestTCP<FutureResponse>( futureResponse, getPeerBean(), getConnectionBean(), message );
    ...
    private class NeighborsRequestTCP<K extends FutureResponse>
    ...
    private class NeighborsRequestUDP<K extends FutureResponse>
