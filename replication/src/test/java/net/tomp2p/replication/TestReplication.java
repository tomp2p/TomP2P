...
import net.tomp2p.dht.FutureShutdown;
...
public class TestReplication {
    ...
    public void testDataloss() throws IOException, InterruptedException {
        ...
        try {
            ...
            p1 = new PeerBuilder(Number160.createHash("111")).setEnableIndirectReplication(true).ports(PORT)
                    .start();
            p2 = new PeerBuilder(Number160.createHash("22")).setEnableIndirectReplication(true).ports(PORT+1)
                    .start();
            p3 = new PeerBuilder(Number160.createHash("33")).setEnableIndirectReplication(true).ports(PORT+2)
                    .start();
            ...
            Thread.sleep(500);
            p3 = new PeerBuilder(locationKey).setEnableIndirectReplication(true).ports(PORT+3).start();
            ...
            p3.bootstrap().peerAddress(p1.peerAddress()).start().awaitUninterruptibly();
            getReplicasCount(locationKey, p1, p2, p3);
            Thread.sleep(500);
    ...
    public void testDataloss2() throws IOException, InterruptedException {
        ...
        try {
            ...
            p3 = new PeerBuilder(locationKey).setEnableIndirectReplication(true).ports(PORT+3).start();
            System.out.println("now we add a peer that matches perfectly the key " + locationKey+ ". This will now become the responsible peer");
            p3.bootstrap().peerAddress(p2.peerAddress()).start().awaitUninterruptibly();
            
            p1 = new PeerBuilder(Number160.createHash("1111")).setEnableIndirectReplication(true).ports(PORT+4)
                    .start();
            p1.bootstrap().peerAddress(p2.peerAddress()).start().awaitUninterruptibly();
    ...
    private static int getReplicasCount(Number160 key, Peer... peers) {
        ...
        for(int i=0; i<peers.length; i++)
            if(!peers[i].isShutdown())
                for(Map.Entry<Number640, Data> entry: peers[i].peerBean().storageLayer().get().entrySet()){
    ...
    public void testSimpleIndirectReplicationForward() throws Exception {
        ...
        try {
            ...
            Thread.sleep(1000);
            //test it
            Assert.assertEquals(true, peers[1].peerBean().storageLayer().contains(new Number640(Number160.createHash("2"), Number160.ZERO, Number160.ZERO, Number160.ZERO)));
    ...
    public void testIndirectReplicationForward() throws Exception {
        ...
        try {
            ...
            final int tests = 10;
            final int wait = 2500;
            while (!peerClose.peerBean().storageLayer()
            ...
            Assert.assertEquals(true, peerClose.peerBean().storageLayer()
                    .contains(new Number640(locationKey, Number160.ZERO, Number160.ZERO, Number160.ZERO)));
        } finally {
            if (master != null) {
                master.shutdown().awaitUninterruptibly();
	...
	public void testShutdown() throws Exception {
		...
		try {
			...
			PeerDHT[] peers = createNodesWithShortId(nrPeers, rnd, port, null, true);
			master = peers[0];
