...
public class PingRPC extends DispatchHandler {
	 ...
	 */
	PingRPC(final PeerBean peerBean, final ConnectionBean connectionBean, final boolean enable,
	...
	public FutureResponse pingUDPProbe(final PeerAddress remotePeer, final ChannelCreator channelCreator,
			final ConnectionConfiguration configuration) {
		final Message message = createRequestMessage(remotePeer, RPC.Commands.PING.getNr(), Type.REQUEST_3);
	...
	private RequestHandler<FutureResponse> createHandler(final PeerAddress remotePeer, final Type type,
			final ConnectionConfiguration configuration) {
		final Message message = createRequestMessage(remotePeer, RPC.Commands.PING.getNr(), type);
	...
	private FutureResponse createDiscoverHandler(final PeerAddress remotePeer, PeerAddress senderAddress) {
		final Message message = createRequestMessage(remotePeer, RPC.Commands.PING.getNr(), Type.REQUEST_2);
