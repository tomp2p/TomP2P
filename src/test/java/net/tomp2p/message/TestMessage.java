...
public class TestMessage {
    ...
    @Test
    public void testEncodeDecode480Map() throws Exception {
        // encode
        Message m1 = Utils2.createDummyMessage();
        ...
        for (int i = 0; i < 1000; i++) {
            dataMap.put(
                    new Number480(new Number160(rnd), new Number160(rnd), new Number160(rnd)),
    ...
    @Test
    public void testEncodeDecode480Set() throws Exception {
        // encode
        Message m1 = Utils2.createDummyMessage();
    ...
    private ChannelBuffer encode(Message m1, boolean tcp) throws Exception {
        DummyCoder coder = new DummyCoder(tcp);
        return coder.encode(m1);
    }

    private Message decode(ChannelBuffer buffer, boolean tcp) throws Exception {
        SocketAddress sockRemote = new InetSocketAddress(2000);
        SocketAddress sockLocal = new InetSocketAddress(1000);
        DummyChannel dc = new DummyChannel(sockRemote, sockLocal);
        if (tcp) {
    ...
    private void compareMessage(Message m1, Message m2) {
        ...
        if (m1.getDataMap480() != null || m2.getDataMap480() != null) {
            ...
            while (it1.hasNext()) {
                ...
                Assert.assertEquals(m1.getDataMap480().get(key1).getLength(), m2.getDataMap480().get(key1).getLength());
