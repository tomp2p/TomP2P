...
public class Utils2
{
        ...
        throws UnknownHostException
    {
        return createDummyMessage( new Number160( "0x4321" ), "127.0.0.1", 8001, 8002, new Number160( "0x1234" ),
                                   "127.0.0.1", 8003, 8004, Command.PING, Type.REQUEST_1, firewallUDP, firewallTCP );
    }
    ...
    public static PeerAddress createAddress( Number160 idSender, String inetSender, int tcpPortSender,
                                             int udpPortSender, boolean firewallUDP, boolean firewallTCP )
        throws UnknownHostException
    {
    ...
    public static Message createDummyMessage( Number160 idSender, String inetSender, int tcpPortSendor,
                                              ...
                                              int tcpPortRecipient, int udpPortRecipient, Command command, Type type,
                                              boolean firewallUDP, boolean firewallTCP )
        throws UnknownHostException
    {
        ...
        //
        PeerAddress n2 =
            createAddress( idRecipien, inetRecipient, tcpPortRecipient, udpPortRecipient, firewallUDP, firewallTCP );
    ...
    public static Peer[] createNodes( int nrOfPeers, Random rnd, int port, int refresh,
                                      boolean enableIndirectReplication )
        throws Exception
    {
        if ( nrOfPeers < 1 )
        {
        ...
        if ( refresh > 0 )
        {
            ...
            {
                peers[0] =
                    new PeerMaker( new Number160( rnd ) ).setRandom( rnd ).setEnableIndirectReplication( enableIndirectReplication ).setTcpPort( port ).setUdpPort( port ).setReplicationRefreshMillis( refresh ).makeAndListen();
            }
            else
            {
                peers[0] =
                    new PeerMaker( new Number160( rnd ) ).setRandom( rnd ).setTcpPort( port ).setUdpPort( port ).setReplicationRefreshMillis( refresh ).makeAndListen();
            }
        }
        else
        {
            ...
            {
                peers[0] =
                    new PeerMaker( new Number160( rnd ) ).setRandom( rnd ).setEnableIndirectReplication( enableIndirectReplication ).setTcpPort( port ).setUdpPort( port ).makeAndListen();
            }
            else
            {
                peers[0] =
                    new PeerMaker( new Number160( rnd ) ).setRandom( rnd ).setTcpPort( port ).setUdpPort( port ).makeAndListen();
            }
        }
        for ( int i = 1; i < nrOfPeers; i++ )
        {
            ...
            {
                peers[i] =
                    new PeerMaker( new Number160( rnd ) ).setEnableIndirectReplication( enableIndirectReplication ).setMasterPeer( peers[0] ).makeAndListen();
            }
            else
            {
        ...
        throws Exception
    {
        ...
        {
            peers[i] =
                new PeerMaker( new Number160( rnd ) ).setEnableMaintenance( false ).setPorts( startPort + i ).makeAndListen();
        }
        System.err.println( "real peers created." );
        return peers;
    }

    public static Peer[] createNonMaintenanceNodes( int nrOfPeers, Random rnd, int port )
        throws IOException
    {
        if ( nrOfPeers < 1 )
        {
        ...
        {
            peers[i] =
                new PeerMaker( new Number160( rnd ) ).setEnableMaintenance( false ).setMasterPeer( peers[0] ).makeAndListen();
        }
    ...
    public static Peer[] createAndAttachNodes( int nr, int port, Random rnd )
        throws Exception
    {
    ...
    public static void findInMap( PeerAddress key, Peer[] peers )
    {
        for ( int i = 0; i < peers.length; i++ )
        {
            ...
            {
                System.out.println( "Peer " + i + " with the id " + peers[i].getPeerID() + " knows the peer " + key );
            }
        }
    }
