...
public class ExpiringMap<K, V> implements ConcurrentMap<K, V> {
	...
	{
		 ExpiringObject answer = delegate.putIfAbsent(key, new ExpiringObject(key, value, System.currentTimeMillis()));
	     if (answer == null) 
	     {
	     	return null;
	     }
	...
	public boolean remove(Object key, Object value)
	{
		...
		ExpiringObject value2 = new ExpiringObject((K)key, (V)value, 0L);
		return delegate.remove(key, value2);
	}
	...
	{
		ExpiringObject oldValue2 = new ExpiringObject((K)key, (V)oldValue, 0L);
		ExpiringObject newValue2 = new ExpiringObject((K)key, (V)newValue, 0L);
		return delegate.replace(key, oldValue2, newValue2);
	}
	...
	{
		ExpiringObject value2 = new ExpiringObject((K)key, (V)value, 0L);
		ExpiringObject answer = delegate.replace(key, value2);
	    if (answer == null) 
	    {
	    	return null;
	    }
