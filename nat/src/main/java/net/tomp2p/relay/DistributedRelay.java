...
public class DistributedRelay {
    ...
    public FutureForkJoin<FutureDone<Void>> shutdown() {
    	...
    	final AtomicReferenceArray<FutureDone<Void>> futureDones2 = new AtomicReferenceArray<FutureDone<Void>>(futureDones);
    	int i=1;
     ...
     * constructor of this class. It is important that we call this after we bootstrapped and have recent information in our peer map.
     * 
        ...
        futureChannelCreator.addListener(new BaseFutureAdapter<FutureChannelCreator>() {
            public void operationComplete(final FutureChannelCreator future) throws Exception {
                if (future.isSuccess()) {
                    ...
                    Set<PeerAddress> relayCandidates = relayCandidates();
                    setupPeerConnections(futureRelay, cc, relayCandidates);
                } else {
	 ...
	 * @return FutureDone
	 */
	private void setupPeerConnections(final FutureRelay futureRelay, final ChannelCreator cc, Set<PeerAddress> relayCandidates) {
		int nrOfRelays = Math.min(PeerAddress.MAX_RELAYS, relayCandidates.size());
		if (nrOfRelays > 0) {
			@SuppressWarnings("unchecked")
     ...
     * @return
     */
    private void setupPeerConnectionsRecursive(final AtomicReferenceArray<FutureDone<PeerConnection>> futures, final Set<PeerAddress> relayCandidates, final ChannelCreator cc, final int numberOfRelays,
        ...
        for (int i = 0; i < numberOfRelays; i++) {
            if (futures.get(i) == null) {
                ...
                futures.set(i, relayRPC.setupRelay(cc, fpc));
                if (futures.get(i) != null) {
                    active++;
                }
            } else {
        ...
        FutureForkJoin<FutureDone<PeerConnection>> ffj = new FutureForkJoin<FutureDone<PeerConnection>>(Math.min(futureRelay.minRelays(), active), false, futures);
        ...
        ffj.addListener(new BaseFutureAdapter<FutureForkJoin<FutureDone<PeerConnection>>>() {
            public void operationComplete(FutureForkJoin<FutureDone<PeerConnection>> futureForkJoin) throws Exception {
                if (futureForkJoin.isSuccess()) {
                    List<FutureDone<PeerConnection>> reponses = futureForkJoin.getCompleted();
