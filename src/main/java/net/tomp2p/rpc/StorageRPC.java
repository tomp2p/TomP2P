    ...
    extends ReplyHandler
{
     ...
     * @param protectDomain Set to true if the domain should be set to protected. This means that this domain is flagged
     *            an a public key is stored for this entry. An update or removal can only be made with the matching
     *            private key.
     * @param protectEntry Set to true if the entry should be set to protected. This means that this domain is flagged
     *            an a public key is stored for this entry. An update or removal can only be made with the matching
     *            private key.
     * @param signMessage Set to true if the message should be signed. For protecting an entry, this needs to be set to
     *            true.
    ...
    public FutureResponse put( final PeerAddress remotePeer, final Number160 locationKey, final Number160 domainKey,
                               final Map<Number160, Data> dataMap, boolean protectDomain, boolean protectEntry,
    ...
    public FutureResponse putIfAbsent( final PeerAddress remotePeer, final Number160 locationKey,
                                       final Number160 domainKey, final Map<Number160, Data> dataMap,
                                       boolean protectDomain, boolean protectEntry, boolean signMessage,
    ...
    /**
     * Compares and puts data on a peer. It first compares the hashes that the user provided on the remote peer, and if
     * the hashes match, the data is stored. If the flag partial put has been set, then it will store those data where
     * the hashes match and ignore the others. This is an RPC.
     * 
     * @param remotePeer The remote peer to store the data
     ...
     * @param signMessage Set to true if the message should be signed. For protecting an entry, this needs to be set to
     *            true.
     * @param partialPut Set to true if partial puts should be allowed. If set to false, then the complete map must
     *            match the hash, otherwise it wont be stored.
     * @param channelCreator The channel creator
     * @param forceUDP Set to true if the communication should be UDP, default is TCP
     * @return FutureResponse that stores which content keys have been stored.
     */
    public FutureResponse compareAndPut( final PeerAddress remotePeer, final Number160 locationKey,
                                         ...
                                         final FutureSuccessEvaluator futureSuccessEvaluator, boolean protectDomain,
                                         boolean protectEntry, boolean signMessage, boolean partialPut,
                                         ChannelCreator channelCreator, boolean forceUDP )
    {
     ...
     * @param domainKey The domain key
     * @param dataMap The map with the content key and data
     * @param type The type of put request, this depends on put/putIfAbsent/protected/not-protected
    ...
    private FutureResponse put( final PeerAddress remotePeer, final Number160 locationKey, final Number160 domainKey,
                                final Map<Number160, Data> dataMap, final Type type, boolean signMessage,
     ...
     * @param locationKey The location key
     * @param domainKey The domain key
     * @param dataSet The set with data. This will be converted to a map. The key for the map is the SHA-1 of the data.
     * @param protectDomain Set to true if the domain should be set to protected. This means that this domain is flagged
     *            an a public key is stored for this entry. An update or removal can only be made with the matching
     *            private key.
     * @param signMessage Set to true if the message should be signed. For protecting an entry, this needs to be set to
    ...
    public FutureResponse add( final PeerAddress remotePeer, final Number160 locationKey, final Number160 domainKey,
                               final Collection<Data> dataSet, boolean protectDomain, boolean signMessage,
                               boolean list, ChannelCreator channelCreator, boolean forceUDP,
                               SenderCacheStrategy senderCacheStrategy )
    {
    ...
    public FutureResponse get( final PeerAddress remotePeer, final Number160 locationKey, final Number160 domainKey,
                               ...
                               final SimpleBloomFilter<Number160> contentBloomFilter, final boolean signMessage,
                               final boolean digest, final boolean returnBloomFilter, final boolean range,
     ...
     * @param domainKey The domain key
     * @param contentKeys The content keys or null if requested all
     * @param sendBackResults Set to true if the removed data should be sent back
     * @param signMessage Adds a public key and signs the message. For protected entry and domains, this needs to be
     *            provided.
    ...
    public FutureResponse remove( final PeerAddress remotePeer, final Number160 locationKey, final Number160 domainKey,
                                  final Collection<Number160> contentKeys, final boolean sendBackResults,
                                  final boolean signMessage, ChannelCreator channelCreator, boolean forceUDP )
    {
        Utils.nullCheck( remotePeer, locationKey, domainKey );
        final Message message =
            createMessage( remotePeer, Command.REMOVE, sendBackResults ? Type.REQUEST_2 : Type.REQUEST_1 );
        if ( signMessage )
        {
            message.setPublicKeyAndSign( getPeerBean().getKeyPair() );
        }
        ...
        throws IOException
    {
        if ( !( message.getCommand() == Command.ADD || message.getCommand() == Command.PUT
            || message.getCommand() == Command.GET || message.getCommand() == Command.REMOVE || message.getCommand() == Command.COMPARE_PUT ) )
        {
        ...
        switch ( message.getCommand() )
        {
            ...
            {
                boolean isNull =
                    Utils.nullCheckRetVal( message.getKeyKey1(), message.getKeyKey2(), message.getDataMap() );
                if ( isNull )
                {
            ...
            case PUT:
            {
                ...
                return handlePut( message, responseMessage, isStoreIfAbsent( message ), isDomainProtected( message ),
                                  isNull );
            }
            case GET:
            {
    ...
    private Message handlePut( final Message message, final Message responseMessage, final boolean putIfAbsent,
                               final boolean protectDomain, final boolean isDataMap480 )
        throws IOException
    {
        ...
        if ( isDataMap480 )
        {
            ...
            for ( Map.Entry<Number480, Data> entry : toStore.entrySet() )
            {
                ...
                if ( doPut( putIfAbsent, protectDomain, publicKey, entry.getKey().getLocationKey(),
                               ...
                               final boolean isDataMap480 )
    {
        if ( isDataMap480 )
        {
            ...
            for ( Map.Entry<Number480, Data> entry : data480.entrySet() )
            {
                ...
                Number160 contentKey =
                    doAdd( protectDomain, entry.getKey().getLocationKey(), entry.getKey().getDomainKey(),
                           entry.getKey().getContentKey(), entry.getValue(), publicKey, list, getPeerBean() );
                if ( contentKey != null )
                {
        ...
        else
        {
            ...
            {
                Number160 contentKey =
                    doAdd( protectDomain, locationKey, domainKey, entry.getKey(), entry.getValue(), publicKey, list,
                           getPeerBean() );
                if ( contentKey != null )
                {
    ...
    private static Number160 doAdd( final boolean protectDomain, final Number160 locationKey,
                                    final Number160 domainKey, final Number160 contentKey, final Data value,
                                    final PublicKey publicKey, final boolean list, PeerBean peerBean )
    {
        if ( list )
        {
            ...
            PutStatus status;
            while ( ( status =
                peerBean.getStorage().put( locationKey, domainKey, contentKey2, value, publicKey, true, protectDomain ) ) == PutStatus.FAILED_NOT_ABSENT )
            {
    ...
    private Message handleGet( final Message message, final Message responseMessage )
    {
        ...
        if ( digest )
        {
            ...
            {
                digestInfo =
                    getPeerBean().getStorage().digest( locationKey, domainKey, keyBloomFilter, contentBloomFilter );
            }
            else
            {
        ...
        else
        {
            ...
            else if ( keyBloomFilter != null || contentBloomFilter != null )
            {
                ...
                SortedMap<Number480, Data> tmp =
                    getPeerBean().getStorage().get( locationKey, domainKey, Number160.ZERO, Number160.MAX_VALUE );
                Number320 lockKey = new Number320( locationKey, domainKey );
                Lock lock = getPeerBean().getStorage().getLockNumber320().lock( lockKey );
                try
                {
    ...
    private Message handleRemove( final Message message, final Message responseMessage, final boolean sendBackResults )
    {
        ...
        {
            result =
                getPeerBean().getStorage().remove( locationKey, domainKey, Number160.ZERO, Number160.MAX_VALUE,
                                                   publicKey );
        }
        if ( !sendBackResults )
        {
                                         ...
                                         boolean protectDomain )
    {
        ...
        final Collection<Number160> result =
            getPeerBean().getStorage().compareAndPut( locationKey, domainKey, hashDataMap, publicKey, partial,
                                                      protectDomain );
