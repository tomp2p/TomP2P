...
public class Message {
    ...
    private final Queue<NumberType> contentReferences = new LinkedList<NumberType>();
    ...
    public Message contentType(final Content contentType) {
        for (int i = 0, reference = 0; i < CONTENT_TYPE_LENGTH; i++) {
            if (contentTypes[i] == null) {
                ...
                contentReferences.add(new NumberType(reference, contentType));
                return this;
	...
	public void restoreContentReferences() {
		...
		for (Content contentType : contentTypes) {
			...
			contentReferences.add(new NumberType(index, contentType));
     ...
     */
    public Queue<NumberType> contentReferences() {
