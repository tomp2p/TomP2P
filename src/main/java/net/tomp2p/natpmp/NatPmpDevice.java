 ...
 * the network. There are two types of messages that can be sent to the device. {@link ExternalAddressRequestMessage}
 * can be sent to get the external IP of the gateway. {@link MapRequestMessage} can be sent to map a port for a certain
 * amount of time. These two messages can be put into the message queue through the {@link #enqueueMessage(Message)}
 ...
 * {@link #NatPmpDevice(boolean)} for details about the shutdown mechanism.
 * 
 * @author flszen
 */
public class NatPmpDevice
{
     ...
     * 
     * @param shutdownHookEnabled Shutting down existing port mappings is a desired behavior; therefore, this value is
     *            required! Refer to {@link #setShutdownHookEnabled(boolean)} for details about what value should be
     *            provided here and how it alters the object's behavior.
     * @throws NatPmpException A NatPmpException may be thrown if the local network is not using addresses defined in
    ...
    /**
     * Shuts down this NatPmpDevice. If the shutdown hook is disabled, this method should be called manually at the time
     * port mappings through the NAT-PMP gateway are no longer needed. If the shutdown hook is enabled, this method is
     * called automatically during Java VM shutdown. When this method is called, if the shutdown hook is enabled, it is
     * automatically disabled. It should be noted that when this method is called manually, it blocks until it
     * completes. If it is desired to shutdown asynchronously, the {@link #shutdownAsync(boolean)} method should be
     * called.
     * 
     * @see #isShutdown()
     * @see #setShutdownHookEnabled(boolean)
    ...
    /**
     * Flag indicates if this NatPmpDevice is shutdown. This method will block if a shutdown is in progress. If you
     * desire to wait for an asynchronous shutdown to complete, please monitor the returned {@link Thread} instead.
     * 
