...
/**
 * The connection pool limits the connection used in the application. If too many connections are open, the application
 * has to wait until a connection is closed.
 * 
 * @author Thomas Bocek
 */
public class ConnectionReservation
{
    ...
    final private Map<ChannelCreator, StackTraceElement[]> debug =
        new ConcurrentHashMap<ChannelCreator, StackTraceElement[]>();
    ...
    public ConnectionReservation( ChannelFactory tcpClientChannelFactory, ChannelFactory udpChannelFactory,
                                  ConnectionConfiguration configuration, MessageLogger messageLoggerFilter,
                                  Statistics statistics, Scheduler scheduler )
    {
    ...
    /**
     * Reserves connections. If a reservation of 5 connection is made, then 5 parallel connections can be made. Until
     * the connections are released, the connections can be closed and reopened. The reservation reserves connections
     * that are created and released immediately.
     * 
     * @param permits The number of connections to be reserved
     * @return The future channel creator object that can be used to create the channels. Returns null if something went
     *         wrong (shutdown, interrupt)
     */
    ...
    /**
     * Reserves connections. If a reservation of 5 connection is made, then 5 parallel connections can be made. Until
     * the connections are released, the connections can be closed and reopened.
     * 
     * @param permits The number of connections to be reserved
     * @param keepAliveAndReuse If the connection should stay open (TCP) for later reuse.
     * @return The future channel creator object that can be used to create the channels. Returns null if something went
     *         wrong (shutdown, interrupt)
     */
    ...
    /**
     * Reserves connections. If a reservation of 5 connection is made, then 5 parallel connections can be made. Until
     * the connections are released, the connections can be closed and reopened. Depending on the thread, this may block
     * or not.
     * 
     * @param permits The number of connections to be reserved
     * @param keepAliveAndReuse If the connection should stay open (TCP) for later reuse.
    ...
    /**
     * Acquires connections. Waits in a loop. If a connection is ready, the wait() gets notified and the acquire() is
     * tried again.
     * 
    ...
    private ChannelCreator reserve0( final int permits, final boolean keepAliveAndReuse, String name )
    {
        ...
        try
        {
            ...
            boolean acquired =
                getReservation().acquire( keepAliveAndReuse ? semaphoreOpen : semaphoreCreating, permits );
            if ( acquired )
            {
                ChannelCreator channelCreator =
                    new ChannelCreator( permits, statistics, messageLoggerFilter, tcpClientChannelFactory,
                                        udpChannelFactory, keepAliveAndReuse, name, Thread.currentThread().getId(),
                                        scheduler );
                if ( logger.isDebugEnabled() )
                {
                    logger.debug( "created channels for Thread " + Thread.currentThread().getName() + "/"
                        + Thread.currentThread().getId() );
                    // we do a bit of debugging here, since this is a dangerous place here that may deadlock upon
                    // shutdown
     ...
     * 
     * @param channelCreator The channelcreator that is not used anymore (or at least partially)
