...
public class ParallelRequestBuilder<K extends FutureDHT<?>> extends
    ...
    public ParallelRequestBuilder<K> add(PeerAddress peerAddress) {
        if (directHits == null) {
        	directHits = new TreeSet<PeerAddress>(peerDht.peer().peerBean().peerMap().createXORAddressComparator());
        }
        if (potentialHits == null) {
        	potentialHits = new TreeSet<PeerAddress>(peerDht.peer().peerBean().peerMap().createXORAddressComparator());
        }
